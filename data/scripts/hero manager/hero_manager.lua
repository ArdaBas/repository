...
function hero_manager_builder:create(game, exists)
  ...
  local hero_manager = {
    enabled = true,
	changing_hero = false,
	facing_hero = nil, -- Name of npc_hero that hero is facing. 
	...
	check_heroes_enabled = false, -- True if there are npc heroes on the map.
	switch_heroes_enabled = true,
	dialog_enabled = false,
	hero_dialog_menu = require("scripts/hero manager/hero_dialog"),
  }
...
---------------------------------------------------------------------------------------
  ...
  -- Check if the hero can talk with another hero. Stop the loop if there is no hero.
  function hero_manager:check_heroes_on_map()
     
	if hero_manager.check_heroes_enabled == false then return end
    local map = game:get_map()
    if map ~= nil then
	  for npc_hero in map:get_entities("npc_hero") do
	  -- If hero is facing npc_hero in same layer and close, enable "hero_talk" and save name of npc_hero in variable.
	    if npc_hero:is_facing_hero() then
		  hero_manager.facing_hero = npc_hero
		  game:set_custom_command_effect("action", "hero_talk")
		  sol.timer.start(game, 50, function() hero_manager:check_heroes_on_map() end)
		  return
		end
	  end
	  -- Otherwise, disable custom action (set it to nil or "custom_carry").
      hero_manager.facing_hero = nil
	  if game:get_custom_command_effect("action") == "hero_talk" then 
	    if game:get_hero().custom_carry ~= nil then game:set_custom_command_effect("action", "custom_carry")
		else game:set_custom_command_effect("action", nil) end
	  end
    end
	-- Restart the check function.
    sol.timer.start(game, 150, function() hero_manager:check_heroes_on_map() end)
  end
  
  -- Call this function to notify the hero manager that the current map has changed. Initializes npc heroes.
  -- Reset some variables and enables/disables hero_manager:check_heroes_on_map() depending on the case.
  function hero_manager:on_map_changed(map) 
    ...
    hero_manager.facing_hero = nil
	if game:get_custom_command_effect("action") == "hero_talk" then game:set_custom_command_effect("action", nil) end
	...
	-- Activate the check npc heroes function, in case there are npc heroes.
	if hero_manager:cardinal(hero_manager.npc_heroes_on_map) == 0 then
	  hero_manager.check_heroes_enabled = false
	elseif hero_manager.check_heroes_enabled == false then 
	  hero_manager.check_heroes_enabled = true
	  hero_manager:check_heroes_on_map()
	end	
	...
	-- is left on the map; otherwise delete the info. The info is stored temporarily in "game.active_maps".
	  if hero_manager:cardinal(hero_manager.npc_heroes_to_switch) > 0 then game.save_between_maps:save_map(map)
	  else game.save_between_maps:forget_map(map)
	  end
  ...
  -- Given a list of properties = {index=..., x=..., y=..., layer=..., direction=..., is_on_team=...,
  -- animation_set=..., animation=...} of some npc_hero,
  -- a new npc_hero is created on the map and returned by the function.
  ...
  function hero_manager:switch_hero()
	...
	hero.custom_carry = npc_properties.custom_carry
  ...
  function hero_manager:get_hero_entity(index) 
    if self.current_hero_index == index then return hero 
	else return game:get_map():get_entity("npc_hero_"..index) end --npc_heroes_on_map[index] end 
  end
